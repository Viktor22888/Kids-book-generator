...
function downloadAsPdf() {
    ...
    try {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>${currentBookData.title}</title>
                <style>
                    body { font-family: Arial, sans-serif; margin: 20px; line-height: 1.6; }
                    .page { page-break-after: always; margin-bottom: 30px; }
                    .page-number { font-size: 12px; color: #666; margin-bottom: 10px; }
                    .page-image { max-width: 100%; height: auto; margin: 10px 0; }
                    .page-text { margin: 15px 0; font-size: 16px; }
                    h1 { text-align: center; margin-bottom: 30px; }
                </style>
            </head>
            <body>
                <h1>${currentBookData.title}</h1>
                ${currentBookData.pages.map(page => `
                    <div class="page">
                        <div class="page-number">Страница ${page.page_number}</div>
                        ${page.image ? `<img class="page-image" src="${page.image}" alt="Иллюстрация">` : ''}
