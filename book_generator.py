...
class BookGenerator:
        ...
        for page_num in range(num_pages):
            try:
                ...
                if generated_text and len(generated_text.strip()) > 20:
                    ...
                    page_text = page_text.replace('**', '').replace('*', '').strip()
    ...
    def generate_book(self, query, num_pages=10):
        print(f"Генерация книги для запроса: {query}, страниц: {num_pages}")
        pages_text = self.generate_text(query, max_length=1000, num_pages=num_pages)
        pages = []
